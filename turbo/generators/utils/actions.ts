 ...
 */
export const bunInstall: PlopTypes.CustomActionFunction = async (
  answers: Partial<{ path: string }>,
) => {
  try {
    const installPath = answers?.path || '.';

    console.log(`Running 'bun install' in ${installPath}...`);
 ...
 */
export const bunCreateExpo: PlopTypes.CustomActionFunction = async (
 ...
 */
export const bunCreateNextApp: PlopTypes.CustomActionFunction = async (
  answers: Partial<{ path: string }>,
) => {
  try {
    // Resolve the target path
