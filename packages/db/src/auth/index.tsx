...
import type { Role } from '../schema/types.js';
...
const server = {
  ...
  async inviteUser(this: void, emailAddress: string, role: Role) {
    logger.trace('Inviting Clerk user', { emailAddress });
    ...
    logger.trace('Clerk user invited', { invitation });
  ...
  async updateUserRole(this: void, userId: string, role: Role | null = 'READER') {
    logger.trace('Updating Clerk user role', { role, userId });

    const client = await clerkClient();

    await client.users.updateUserMetadata(userId, { privateMetadata: { role } });

    logger.trace('Clerk user role updated');
  },
  ...
  async withClerkUser<Result, OnDoesNotExist = undefined>(
    ...
    logger.trace('Starting With Clerk user...');
    ...
    logger.success('Clerk user', { user });

    const result = await next(user);

    logger.done('With Clerk user result', { result });
