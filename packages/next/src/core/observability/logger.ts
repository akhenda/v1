...
import { type LogLevel, createProjectLogger } from '@v1/logging/nextjs';

import { ErrorMonitoring } from './monitoring';
import type { LoggerDetails, UserMessageType } from './types';

const Logger = createProjectLogger('@v1/next');
...
function showToast(
  ...
  if (userMessage && Boolean(userMessage.title) && Boolean(userMessage.message)) {
    toast[type](userMessage.title, { description: userMessage.message });
  }
}
