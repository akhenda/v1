...
export const getPlanByKey = internalQuery({
  args: { key: schema.tables.plans.validator.fields.key },
...
export const getOnboardingCheckoutUrl = action({
  handler: async (ctx) => {
    ...
    const product = await ctx.runQuery(internal.subscriptions.getPlanByKey, { key: 'free' });
...
export const getProOnboardingCheckoutUrl = action({
  args: { interval: schema.tables.subscriptions.validator.fields.interval },
  handler: async (ctx, args) => {
    const user = await ctx.runQuery(api.users.getUser);
    if (!user) throw new Error('User not found');

    const product = await ctx.runQuery(internal.subscriptions.getPlanByKey, { key: 'pro' });
    const price =
      args.interval === 'month' ? product?.prices.month?.usd : product?.prices.year?.usd;
...
export const getPolarEventUser = internalQuery({
  args: { polarId: v.optional(v.string()), email: v.string() },
  handler: async (ctx, args) => {
    ...
    if (!user) throw new Error('User not found');
    if (user.polarId) throw new Error(`User ${user.email} already has a Polar ID`);
...
export const replaceSubscription = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const plan = await ctx.db
      .query('plans')
      .withIndex('polarProductId', (q) => q.eq('polarProductId', args.input.productId))
      .unique();
    ...
    if (!user?.polarSubscriptionPendingId) return;

    await ctx.scheduler.cancel(user.polarSubscriptionPendingId);
    await ctx.db.patch(args.userId, { polarSubscriptionPendingId: undefined });
  },
});
...
export const setSubscriptionPending = mutation({
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(userId, { polarSubscriptionPendingId: scheduledFunctionId });
  },
});
