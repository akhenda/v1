...
const handleUpdateSubscription = async (
  ctx: ActionCtx,
  user: Doc<'users'>,
  subscription: WebhookSubscriptionCreatedPayload | WebhookSubscriptionUpdatedPayload,
) => {
...
const handleSubscriptionChange = async (
  ...
  const freePlan = await ctx.runQuery(internal.subscriptions.getPlanByKey, { key: 'free' });

  // Only send email for paid plans
  if (event.data.productId !== freePlan?.polarProductId) {
    await sendSubscriptionSuccessEmail({ email: user.email, subscriptionId: event.data.id });
  }
...
const handlePolarSubscriptionUpdatedError = async (
  ...
  // Only send email for paid plans
  if (event.data.productId !== freePlan?.polarProductId) {
    await sendSubscriptionErrorEmail({ email: user.email, subscriptionId: subscription.id });
  }
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    const event = wh.verify(body, Object.fromEntries(request.headers.entries())) as
