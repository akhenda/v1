...
export default function BillingSettings() {
  ...
  const getUpgradeCheckoutUrl = useAction(api.subscriptions.getProOnboardingCheckoutUrl);
                ...
                {plan._id !== freePlan?._id && (
                  <div className="flex items-center gap-2 px-4">
                    ...
                    <Switch
                      ...
                      onCheckedChange={() =>
                        setSelectedPlanInterval((prev) => (prev === 'month' ? 'year' : 'month'))
                      }
                    />
                  </div>
                )}
              ...
              <div className="flex w-full flex-col items-start p-4">
                <div className="flex items-end gap-2">
                  ...
                  <p className="flex items-start gap-1 text-sm font-normal text-primary/60">
                    ...
                    on:{' '}
                    {new Date(user.subscription.currentPeriodEnd ?? 0 * 1000).toLocaleDateString(
                      'en-US',
                    )}
                    .
                  </p>
