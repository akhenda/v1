...
function handleLoggerPayload(level: LogLevel, message?: string, details?: LoggerDetails) {
  const logger = Logger[level];
  const { error, userMessage, ...rest } = details || {};

  if (message) logger(message, error ? error : '', R.isEmpty(rest) ? '' : rest);
  showToast(userMessage);
}
...
function reporter(level: LogLevel, message?: string, details?: LoggerDetails) {
  ...
  handleLoggerPayload(level, message, details);
